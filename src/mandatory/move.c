...
/*   Updated: 2024/08/25 02:11:55 by nfradet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_game *game)
{
	...
	if ((int)(next_y + 0.2 * get_sign(game->player.dir_y)) > 0 &&
		(int)(next_y + 0.2 * get_sign(game->player.dir_y)) < game->map.height - 1)
	{
		if (can_i_go(game, (int)(next_y + 0.2 * get_sign(game->player.dir_y)),
			(int)game->player.x) == 1)
			game->player.y = next_y;
	}
	if ((int)(next_x + 0.2 * get_sign(game->player.dir_x)) > 0 && 
		(int)(next_x + 0.2 * get_sign(game->player.dir_x)) < game->map.width - 1)
	{
		if (can_i_go(game, 
			(int)game->player.y,
...
void	move_left(t_game *game)
{
	...
	if ((int)(next_y - 0.2 * get_sign(game->player.dir_x)) > 0 && 
		(int)(next_y - 0.2 * get_sign(game->player.dir_x)) < game->map.height - 1)
	{
		if (can_i_go(game, 
	...
	if ((int)(next_x + 0.2 * get_sign(game->player.dir_y)) > 0 && 
		(int)(next_x + 0.2 * get_sign(game->player.dir_y)) < game->map.width - 1)
	{
		if (can_i_go(game, 
			(int)game->player.y,
